...
function SearchOutcome(props) {
  ...
  const filteredAreaData = props.data.filter(areaData => Ctx.selectedArea.includes(areaData.sarea))
  useEffect(() => {
  ...
  return (
    <div className={style.SearchOutcomeContainerStyle}>
      <ul className={style.SearchOutcomeStyle}>
        ...
        <SearchOutcomeList
          sarea={item.sarea} sna={item.sna} sbi={item.sbi} bemp={item.bemp}
          index={index}
        />) : null}
      </ul>
...
export const SearchOutcomeList = (props) => {
  return (
    <li className={style.SearchOutcomeListStyle} style={{ backgroundColor: props.index % 2 === 1 ? '#F6F6F6' : 'white' }}>
