  ...
  return (
    <>
      ...
      <section className={style.searchSectionStyle}>
        <select
          className={style.searchSelectStyle}
          value={city}
          ...
          {searchStation.length !== 0 ? (
            <div className={style.searchStationOutcomeStyle}>
              {searchStation.map((station) => {
                return (
                  <div
                    className={style.searchStationOutcomeListStyle}
                    onClick={() => searchOutcomePickHandler(station.sna)}
                  >
                    {station.sna}
                  </div>
                );
      ...
      <section style={{ display: "flex" }}>
        <div>
          ...
          <div className={style.searchSelectAreasStyle}>
            {areas[city].map((area) => (
              <CheckBox name={area} allIsActive={Ctx.allIsActive} />
            ))}
          </div>
        </div>
        <img src="https://imgur.com/JeRjaEM.jpg" className={style.searchImg} />
      </section>
    </>
  );
}
